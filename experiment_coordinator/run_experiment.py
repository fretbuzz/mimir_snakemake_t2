...
def main(experiment_name, config_file, prepare_app_p, port, ip, localhostip, install_det_depen_p, exfil_p):
    ...
    for i in range(0, int(config_params["experiment"]["number_of_trials"])):
        ...
        subprocess.Popen(['python', './src/pod_creation_looper.py', './experimental_data/' + exp_name + '_pod_creation_log.txt', './' + end_sentinal_file_loc], shell=False, stdin=None, stdout=None, stderr=None, close_fds=True)
        subprocess.Popen(['bash', './src/hpa_looper.py ', str(int(math.ceil(float(experiment_length)/60))),
                          ' ./experimental_data/' + exp_name + '_hpa_log.txt'],  shell=False, stdin=None, stdout=None,
