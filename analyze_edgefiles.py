...
# (5) try running the dockerGuard code
    ...
    if calc_vals_p:
        for file_path in filenames:
            ...
            # note: okay, gotta correct a 'problem here'
            # want: class A -> vip B -> vip A -> class B
            # but what I get: class A -> vip B ; vip A -> class B
            # now adding the link is a little trickier than you'd expect b/c we could have multiple
            # of these pairings... but the key thing is we deterministically know the second hop via the first hop
            '''
            for (u, v, data) in G.edges(data=True):
                # print (u,v,data)
                if 'VIP' in v and u in ms_s:
                    # first off, find VIP of u
                    vip_of_u = ms_to_vip[u]
                    # then can add another edge (unless one already exists, then increase wieght
                    if G.has_edge(v, vip_of_u):
                        G[v][vip_of_u]['weight'] += data['weight']
                    else:
                        G.add_edge(v, vip_of_u, weight=data['weight'])
            '''
            # even if a node does not communicate -> still want it in the graph
            # note that I'll need to adjust this for when I dynamically modify the # of nodes
            # (prob by making container_to_ip into a list)
            ...
            for container_list_and_class in container_to_ip.values():
                ...
                for ms in ms_s:
                    if ms in container_list_and_class[0] and 'VIP' not in container_list_and_class[0]:
                        if container_list_and_class[0] not in G:
                            G.add_node(container_list_and_class[0])
                            break
            plt.clf()
            plt.figure(figsize=(20, 8))
            pos = nx.spring_layout(G, k= 2 * (1/math.sqrt(len(list(G)))))
