...
Running physical exfiltration events on the deployed application is currently *not* supported by the 
...
mimir_v2/configs_to_reproduce_results/Data_Collection directory. Convenience scripts to collect this data
are provided and are described below.
...
mentioned convenience scripts include the commands to run this analysis.

**Convenience Script:** There's convenience scripts provided that perform the complete end-to-end workflow of 
collecting data, analyzing it, and forming the graphs in the paper. They install all necessary
dependencies, setup Minikube, deploy the application, simulate user traffic and record network activity
(several times), and then analyze all of the collected data. Modifying part of this 
...
# each of these scripts create one/two of the figures from the paper. Each of these will take 4-6 days to run probably.
