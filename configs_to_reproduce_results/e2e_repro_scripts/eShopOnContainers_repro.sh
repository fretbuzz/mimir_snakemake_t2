...
## TODO: add in autoscaling... (will probably need to manually just try scaling the various parts...)
# (based off of diagram from repo page, looks like we can scale everything but the DBs...)
# it looks like lots of microservice are bundled with their DBs... so we cannot scale those, also probably
# shouldn't scale RabbitMQ, so I guess just scale the front-facing microservice...
